...
canvas.width = mw * W+2 + 200;
canvas.height = mh * H+2;
...
let display = () => {
	...
	exportedJSON.forEach(({x, y, w, h, specw, spech, isPower, name}) => {
		...
		if(specw > 5 && spech > 5)
			ctx.font = '14px Arial';
		ctx.fillText(name+(w==specw ? '' : '*')+' '+specw+'x'+spech, x*W + w*W/2, y*H + h*H/2);
	});
...
canvas.onmousemove = (e) => {
	...
	if(x < mw){
		...
		if(firstPosition){
			...
			ctx.globalAlpha = 1;
			ctx.fillText(JSON.stringify([zx-ax,zy-ay]), canvas.width - 100, 50);
		}else
